;(function () {
    ...
    */
    ...
    //FastClick使用之trigger触发click失效（新增）
...
/*获取短信验证码*/
function getMsgCode(el,time,url){
    var defaults = {
        el:el,
        time:time || 60,
        url:url || '',
    ...
    var i = opts.time,btn = $(opts.el),timer;
    btn.click(a);
    function a(){
        btn.off('click').text(i+'秒后重新获取').removeClass('on');
        timer = setInterval(b,1000);
        $.post(opts.url,function(res){
    ...
    function b(){
        i--,btn.text(i+'秒后重新获取');
        if(i<=0){
            btn.text('获取验证码').addClass('on');
            clearInterval(timer),btn.click(a);
            i = opts.time;
        }
    };
};